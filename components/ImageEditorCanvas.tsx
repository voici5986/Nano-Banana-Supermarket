    ...
    <div className="flex flex-col gap-4">
        ...
        >
            {!initialImageUrl ? (
                <label htmlFor="file-upload" className="flex flex-col items-center justify-center text-gray-500 cursor-pointer w-full h-full">
                    ...
                    <p className="mb-2 text-sm"><span className="font-semibold">点击上传</span> 或拖拽图片</p>
        ...
        {initialImageUrl && isMaskToolActive && (
            <div className="p-3 bg-black/60 backdrop-blur-md rounded-lg flex flex-col gap-4 border border-white/10 animate-fade-in-fast">
                <p className="text-xs text-gray-400 -mb-2">在图像上绘制以创建蒙版，用于局部编辑。</p>
                <div className="flex items-center gap-4">
                    <label htmlFor="brush-size" className="text-sm font-medium text-gray-200 whitespace-nowrap">笔刷大小</label>
                ...
                <div className="grid grid-cols-2 gap-2">
                    <button onClick={handleUndo} disabled={history.length === 0} className="px-4 py-2 text-sm font-semibold text-gray-200 bg-gray-800 rounded-md hover:bg-gray-700 disabled:bg-gray-800 disabled:text-gray-500 disabled:cursor-not-allowed transition-colors">撤销</button>
                    <button onClick={clearMask} disabled={history.length === 0} className="px-4 py-2 text-sm font-semibold text-gray-200 bg-gray-800 rounded-md hover:bg-gray-700 disabled:bg-gray-800 disabled:text-gray-500 disabled:cursor-not-allowed transition-colors">清除遮罩</button>
                </div>
            </div>
        )}
    </div>
